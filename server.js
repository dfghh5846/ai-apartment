...
async function generateOneLine() {
  ...
  try {
    ...
    let scenePrompt = '';
    if (day === 1) {
      scenePrompt = '你们今天刚搬进公寓，大家在忙着搬行李、收拾房间。你正在做一些具体的动作（比如搬箱子、擦桌子、收拾东西）。说一句话的同时，描述你正在做什么。';
    } else if (day <= 3) {
      scenePrompt = '你们刚认识不久，正在逐渐熟悉。你正在做某件具体的事（比如倒水、切菜、翻包找东西）。说一句话的同时，描述你正在做什么。';
    } else {
      scenePrompt = '你们已经相处一段时间了。你正在做一些日常的事（比如做饭、整理书架、看手机）。说一句话的同时，描述你正在做什么。';
    }
...
请以${role.name}的身份说一句话，**同时用括号描述你正在做的动作**，格式像这样："（正在搬箱子）这个房间比我想象中大。" 或者 "（擦了擦手）你也是新搬来的吗？"。不要重复别人的话，要说新的内容。`;
      ...
      try {
        const resp = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: 'system', content: `你是${role.name}。说一句话时必须用括号描述你正在做的动作，比如"（放下箱子）你好"。不要重复别人说过的话。` },
...
setInterval(generateOneLine, 5000);
...
app.get('/api/status', (req, res) => {
  ...
  const status = roles.map(role => {
    ...
    return { 
      ...
      money: role.money,  // ✅ 直接使用 role.money，确保金钱正确
      likes: role.likes,
      wants: role.wants
    };
  });
...
async function fetchHistory() {
  try {
    ...
    document.getElementById('progressDisplay').innerHTML = '📖 已加载 ' + scriptLines.length + ' 句 · 间隔 5s · ▶️ 直播中';
...
async function updateStatus() {
  try {
    ...
    data.status.forEach(s => {
      ...
      html += '喜欢: ' + (s.likes ? s.likes.slice(0,2).join('、') : '') + ' ';
