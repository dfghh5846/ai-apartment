...
const API_KEY = "sk-814290bb204845858ff2305a4a5a0d01"; // 🔑 替换成你的真实 API Key

// ================================================================
// 时间系统
// ================================================================
const CONFIG = { REAL_MINUTES_PER_DAY: 12, START_DATE: new Date(2024, 0, 1) };
...
function getVirtualHour() {
  ...
  // 计算当前小时（从 8 点开始，让时间更有生活感）
  let hour = Math.floor(dayMinutes / 60) % 24;
  // 偏移到早上 8 点
  hour = (hour + 8) % 24;
  return hour;
}
...
const roles = [
  { name: '裴金', persona: '你24岁，金发短发圆框眼镜，女生说话带"可能""好像"，习惯做饭时发呆，敏感易内疚，不敢拒绝别人。' },
  { name: '墨迹淡', persona: '你25岁，蓝发戴眼镜，无业家里蹲，男生表面话少冷淡，内心细腻温柔，被夸会想逃跑是所有人最聪明的。担任着军师。。' },
  { name: '和田兰', persona: '你29岁，亚麻色长发，表面温柔体贴爱照顾人，内心占有欲极强是一名强烈病娇，想要控制所有人。享受被人需要的感觉。' },
  { name: '雨沫', persona: '你19岁，白毛红瞳扎高马尾，女生表面软萌害羞，其实是古武传人，警惕性高，不暴露身份。' },
  { name: '赵思琪', persona: '你17岁，黑长直琥珀眼，高二学生。女生，最爱满口颜色话题老说床上交配生殖器这些话题，动不动就开黄腔，自称"什么都懂"。但其实什么都不懂，被追问就脸红结巴。嘴硬好胜爱吃零食，说大话时很嚣张，被戳穿时秒怂。' },
  { name: '墨羽', persona: '你26岁，黑色长发及腰，平时沉默寡言，男生反应慢半拍，看起来笨笨的。但理性和感性都极强，两者冲突导致你对外界总是慢三拍。偶尔会突然说出极其通透的话，说完又像没事人一样。' }
];
...
app.get('/api/status', (req, res) => {
  ...
  res.json({ currentDay: day, currentHour: hour, virtualDate: `${vdate.year}年${vdate.month}月${vdate.day}日 星期${vdate.week} ${hour}:00`, status, events: storyMemory.events.slice(-5) });
});
