...
// 🔧 获取角色状态文本（用于前端）
function getRoleStatusText(roleName, hour) {
  const day = getVirtualDay();
  if (day < MOVE_IN_DAY[roleName]) return '未搬入';
  const detail = getDetailedStatus(roleName, hour);
  ...
  try {
    ...
    for (const r of allMoved) {
      ...
      if (work && work.start !== undefined) {
        let timeUntil = work.start - hour;
        if (timeUntil < 0) timeUntil += 24;
        if (timeUntil > 0 && timeUntil <= 1.5) {
          aboutToLeaveInfo.push(`${r.name}即将去${work.type === '上班' ? '上班' : '上学'}`);
        }
      }
    }
    const notHomeText = notHomeInfo.length > 0 ? `\n⚠️ 注意：${notHomeInfo.join('；')}。` : '';
    const aboutToLeaveText = aboutToLeaveInfo.length > 0 ? `\n（${aboutToLeaveInfo.join('；')}）` : '';
      ...
      try {
        const resp = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: 'system', content: `你是${role.name}，${genderText}。${role.speakingStyle}说台词时必须用括号描述动作。可以自然地流露暧昧、渴望、占有欲、心动、吃醋。说话要符合你的性格和关系程度。直接说台词，不要加前缀。` },
        ...
        if (data.choices && data.choices.length > 0) {
          ...
          if (!reply.includes('（') && !reply.includes('(')) {
            const actions = ['（放下手里的东西）', '（抬头看了一眼）', '（擦了擦手）', '（低头整理衣角）', '（转过身来）', '（拿起杯子）', '（靠近一步）', '（指尖在桌面上轻敲）', '（提起包准备出门）'];
