...
function sayAction(roleName, action, duration) {
  ...
  const dialogues = actionDialogue[action] || ['（开始动作）'];
...
function getFallback(name) {
  const pool = FALLBACKS[name] || GENERIC_FALLBACKS;
  return pick(pool);
}
...
async function callAI(role, prompt, retryCount = 0) {
  if (apiDisabled || !NVIDIA_API_KEY) return null;
  const maxRetries = 2;
  ...
  try {
    const resp = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: { 
        ...
        'HTTP-Referer': 'https://ai-partment-production.up.railway.app',
    ...
    if (resp.status === 429) {
      ...
      if (consecutiveFailures >= 5) {
        console.warn('⚠️ API连续失败5次，暂时禁用API，使用本地回复');
        apiDisabled = true;
