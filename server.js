...
function getDetailedStatus(roleName, hour) {
  ...
  if (work && work.start !== undefined && work.end !== undefined) {
    if (isInTimeRange(hour, work.start, work.end)) {
      if (work.type === '上班') {
        return { status: '上班', icon: '💼', detail: work.description || '工作中' };
      } else if (work.type === '上学') {
        return { status: '上学', icon: '📚', detail: '在学校上课' };
      }
    }
  }
  ...
  if (roleName === '裴金' && work && work.type === '在家办公') {
    return { status: '在家办公', icon: '💻', detail: '线上咨询中' };
  }
...
const roles = [
  {
    ...
    loveStyle: '被动依赖型——被人坚定选择时会逐渐打开心扉。喜欢被记住细节。',
    money: 3200,
    likes: ['做饭', '被夸奖', '被记住细节'],
  ...
  {
    name: '墨迹淡',
    persona: '25岁男生，蓝发戴眼镜，曾是美术天才，因父母离婚自我放弃后变成家里蹲。表面冷淡，内心细腻。',
    speakingStyle: '说话极简，经常沉默半拍才接话。关心别人时用"顺便""刚好"这种别扭方式。',
    loveStyle: '外冷内热别扭型——嘴上不说但行动会默默付出。',
    money: 18000,
    likes: ['画画', '弹吉他', '看窗外发呆'],
  ...
  },
  {
    ...
    loveStyle: '迟钝守护型——自己意识不到感情，但会本能地保护对方。',
    money: 5000,
    likes: ['擦剑', '观察别人', '吃布丁'],
  ...
  },
  {
    ...
    speakingStyle: '满口"操""干"这种词，但说的时候眼神会躲。被人撩一句就结巴。',
    loveStyle: '口是心非纯情型——嘴上什么都敢说，实际连牵手都紧张。',
    money: 800,
    likes: ['吃零食', '说骚话', '跟雨沫斗嘴'],
    wants: ['想学真本事', '想被人在乎'],
