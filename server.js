...
const roles = [
  ...
  {
    name: '赵思琪',
    persona: '17岁女生，黑长直高二。极度好胜嘴硬，满口粗鄙虎狼之词，天天鸡巴和逼自称老司机。但其实初吻还在，完全不懂，被人撩一句就脸红结巴。她用最脏的话来掩饰自己的纯情和不安。',
...
async function generateOneLine() {
  ...
  try {
    ...
    if (day === 1) {
      topicRestriction = '⚠️ 非常重要：今天是第一天，你们是第一次见面，彼此完全不认识。只能聊：自我介绍、问对方名字、问房间在哪、问吃没吃饭。绝对不要聊任何需要住下来才知道的事情（比如热水器、网络、室友习惯等）。也绝对不要提到"昨天"或"以前"。';
    } else if (day <= 3) {
      topicRestriction = '你们刚认识不久，还在互相了解的阶段。话题可以稍微放开，但不要太深入。';
    } else {
      topicRestriction = '你们已经认识一段时间了，可以正常聊天。';
    }
...
请以${role.name}的身份说一句简短的日常对话（10-20字），像真人聊天一样自然。第1天要说符合初次见面的话，不要太过熟悉。`;
      ...
      try {
        const resp = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: 'system', content: `你是${role.name}，正在和第一次见面的室友说话。不要说"昨天"或"以前"，不要提任何需要住下来才知道的事。直接说台词。` },
    ...
    if (!reply || reply.length < 2) {
      const fallbacks = ['你好，我是新搬来的。', '请问房间在哪？', '你也是住这里的吗？', '我叫裴金，请多关照。', '这个公寓挺大的。', '你住多久了？', '我叫墨迹淡。', '我是和田兰，以后请多关照。'];
