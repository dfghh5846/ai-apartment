...
const roles = [
  {
    ...
    roomDesc: '床单是浅粉色的，枕头边放着一只旧布熊，书桌上摆着一盆快枯的绿萝，窗帘是白色的纱，透进来软软的光。'
  },
  {
    ...
    roomDesc: '一张干净到几乎空白的桌子，只有一台笔记本和一支笔。床是深灰色的，墙上贴着一张画了一半的星空图。'
  },
  {
    ...
    roomDesc: '床上铺着白色床单，枕边放着一本《内家拳法》。墙上挂着一把木剑，窗台上摆着几枚磨光的石子。'
  },
  {
    ...
    roomDesc: '窗帘是深红色的，床上堆着好几个抱枕。梳妆台上摆满了瓶瓶罐罐，空气中有一股淡淡的甜香味。'
  },
  {
    ...
    roomDesc: '墙上贴满了动漫海报，书桌上摆着几本言情小说和一支没用过的口红。床单是碎花的，枕头下压着一张写了一半的情书。'
  },
  {
    ...
    roomDesc: '墙上贴满了"追尾人"的剪报和手绘地图，床上放着游戏角色的抱枕。窗帘拉着，整个房间像秘密基地。'
  },
  {
    ...
    roomDesc: '房间出奇地空，只有一张床、一张书桌、一把椅子。桌上放着一本摊开的《道德经》，旁边搁着一支笔。'
  }
];
...
let SERVER_START = new Date('2025-01-01T00:00:00Z').getTime();
...
function decideAction(role, day, hour) {
  ...
  if (state.hunger < 25) return { action: 'EATING', duration: ACTION_DURATION.EATING };
  if (state.thirst < 25) return { action: 'DRINKING', duration: ACTION_DURATION.DRINKING };
...
function startAction(roleName, action, duration) {
  ...
  const actionMessages = {
    SLEEPING: [
      `${roleName}打了个哈欠，眼皮直打架，嘟囔着“不行了...我去睡了...”`,
      `${roleName}揉着眼睛往卧室走，“谁也别吵我...”`,
      `${roleName}一头扎进被窝，“晚安...世界...”`
    ],
    EATING: [
      `${roleName}肚子咕咕叫，快步走向厨房，“饿死了，今天吃什么？”`,
      `${roleName}摸着肚子望向厨房，“再不吃饭我要啃桌子了...”`,
      `${roleName}冲到饭桌前，“我先开动了！”`
    ],
    DRINKING: [
      `${roleName}拿起水杯吨吨吨喝了几大口，“啊...活过来了...”`,
      `${roleName}口渴得不行，赶紧去找水。`
    ],
    TOILET: [
      `${roleName}脸色一变，捂着肚子快步走向卫生间，“憋不住了...”`,
      `${roleName}一声不吭直奔洗手间。`
    ],
    RESTING: [
      `${roleName}瘫在沙发上一动不动，“让我躺会儿...”`,
      `${roleName}闭上眼睛靠在椅背上，“世界太吵了...需要静静...”`
    ],
    SOCIAL: [
      `${roleName}凑到别人身边，“嘿，聊会儿天呗？”`,
      `${roleName}笑着拍拍旁边的座位，“过来坐嘛。”`
    ],
    CLEANING: [
      `${roleName}撸起袖子开始擦擦擦，“这地方怎么这么脏？”`,
      `${roleName}哼着歌收拾房间，“干干净净才舒服。”`
    ],
    HOBBY: [
      `${roleName}拿出书/画笔/乐器，露出满足的笑容，“终于可以享受一下了...”`,
      `${roleName}沉浸在兴趣里，忘掉了所有烦恼。`
    ],
    WALKING: [
      `${roleName}伸了个懒腰，“闷死了，出去透透气。”`,
      `${roleName}一边散步一边哼着小调。`
    ],
    READING: [
      `${roleName}翻开书页，眼神专注，“这本书真有意思...”`,
      `${roleName}捧着书本，完全沉浸在故事里。`
    ],
    WRITING: [
      `${roleName}拿起笔涂涂写写，“嗯...这句怎么改好呢...”`,
      `${roleName}对着笔记本发呆，突然灵光一闪。`
    ]
  };
...
function getIntroLine(name) { const map = { '裴金': '大家好…我、我是裴金…请多关照…', '墨迹淡': '……墨迹淡。', '何田兰': '大家好呀～我是何田兰，以后我来做饭哦～', '雨沫': '各位好，我是雨沫。', '赵思琪': '嗨～想姐姐了吗？嘿嘿～', '唐吉柯德': '追尾人的勇士们！我来了！', '墨尾': '……嗯，我是墨尾。' }; return map[name] || '大家好。'; }
...
async function generateOneLine() {
  ...
  try {
    ...
    if (day === 1 && !introductionDone) {
      history = [
        '裴金：那个…大家好，我叫裴金…以后请多关照…（声音越来越小）',
        '墨迹淡：……墨迹淡。（简短点头）',
        '何田兰：哎呀别紧张～我是何田兰，以后咱们就是室友啦！今晚我下厨？',
        '裴金：啊…不、不用麻烦你…我、我自己解决…（慌乱）',
        '墨迹淡：……随便。（走到窗边）',
        '何田兰：（笑）别客气嘛！我去看看厨房有什么。',
        '裴金：那…我帮你洗菜吧…（小声）',
        '墨迹淡：（侧头）……饭好了叫我。'
      ];
    ...
    const talkers = available.filter(r => { const state = roleStates[r.name]; return state && (!state.action || state.action === 'SOCIAL' || state.actionTimer <= 0) && !state.isSleeping; });
    ...
    let eventText = ''; for (const ev of todayEvents) { if (ev.type === 'birthday') { if (ev.name === role.name) eventText += '🎂 今天是你的生日！大家可能会为你庆祝。'; else eventText += `🎂 今天是 ${ev.name} 的生日！你可以说些祝福的话。`; } else if (ev.type === 'holiday') eventText += `🎉 今天是 ${ev.name}！是个值得庆祝的日子。`; }

    const prompt = `今天是公寓第 ${day} 天，${time.str}。天气：${currentWeather}，${getSeason(day)}季。${eventText}你是${role.name}，${role.gender}，身高${role.height}，${role.appearance}。性格：${role.persona}。说话风格：${role.speakingStyle}。当前状态：${activity.status}${activity.detail ? '：' + activity.detail : ''}。饥饿：${Math.round(state.hunger)}，口渴：${Math.round(state.thirst)}，疲劳：${Math.round(state.fatigue)}，存款：${state.money}元。你的房间：${role.roomDesc}。你的长期目标：${role.goals.join('、')}。${context ? '对话历史：\n' + context : '刚住在一起。'}请用非常自然、口语化的中文说一句话（15-35字），像真人聊天一样，可以带情绪、抱怨、调侃。如果今天有特殊日子，自然地提到它。你可以直接说出你现在想做的事（比如“好饿，我去翻冰箱”、“累死了，我躺会儿”、“想找人说话”），但不要用括号或旁白。`;
    ...
    if (Math.random() < 0.03) { const evts = ['停电半小时', '水管爆裂', '快递堆积', '邻居敲门']; const e = pick(evts); storyMemory.events.unshift(`⚡ 事件：${e}`); timelineLog.push(`⚡ ${e}`); }
