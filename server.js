...
let SERVER_START = new Date('2025-01-01T08:00:00Z').getTime(); // 第1天早上8点开始
...
const DAY_START = new Date('2025-01-01T08:00:00Z').getTime();
...
function getVirtualDate(day) { const base = new Date('2025-01-01T08:00:00Z'); const d = new Date(base.getTime() + (day - 1) * 24 * 60 * 60 * 1000); const weekdays = ['日', '一', '二', '三', '四', '五', '六']; return { year: d.getFullYear(), month: d.getMonth() + 1, day: d.getDate(), week: weekdays[d.getDay()], dateStr: `${d.getMonth()+1}-${d.getDate()}` }; }
...
function getAvailableRoles(day, hour) {
  ...
  if (day === 1) return allMoved; // 第1天不上班
...
function sayAction(roleName, action, duration) {
  ...
  history.push(`${roleName}：${dialogue}`);
  ...
  try {
    ...
    for (const role of available) {
      ...
      if (!state.action || state.actionTimer <= 0) {
        ...
        if (decision) sayAction(role.name, decision.action, decision.duration);
      }
    }

    if (day === 1 && !introductionDone) {
      history = [
        '裴金：那个…大家好，我叫裴金…请多关照…（声音越来越小）',
        '墨迹淡：……墨迹淡。（简短点头）',
