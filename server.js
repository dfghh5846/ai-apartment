...
总字数控制在60字以内。`;
    ...
    try {
      ...
      if (!text || text.length < 2) {
        ...
        const fullLine = `${role.name}：${fallback}`;
...
// 🔧 修改点：清空按钮只清本地显示，不调用服务器接口
async function clearHistory(){
  if(!confirm('确定要清空你本地的聊天记录显示吗？其他人不受影响。'))return;
  lastLength=0;
  story.innerHTML='<div class="empty-state">📭 已清空本地显示，重新加载中...</div>';
  count.textContent='0';
