...
function getDetailedStatus(roleName, hour) {
  ...
  if (work && work.start !== undefined && work.end !== undefined) {
    if (isInTimeRange(hour, work.start, work.end)) {
      const typeMap = { '上班': '上班', '上学': '上学' };
      return { status: typeMap[work.type] || '外出', icon: '🚶' };
    }
  }
...
const roles = [
  {
    name: '裴金',
    persona: '24岁女生，金发短发圆框眼镜，线上咨询师。极度敏感自卑，总觉得自己不配被爱。内心渴望被坚定选择。容易被温柔打动，但也会因为害怕受伤而退缩。',
    speakingStyle: '说话小声犹豫，经常说到一半改口。习惯用"可能""好像""也不是…"这类词。被撩会脸红不知所措。',
    loveStyle: '被动依赖型——被人坚定选择时会逐渐打开心扉。喜欢被记住细节。在感情里很容易陷进去，但又害怕受伤。',
    money: 3200,
    likes: ['做饭', '被夸奖', '被记住细节'],
  ...
  {
    name: '墨迹淡',
    persona: '25岁男生，蓝发戴眼镜，曾是美术天才，因父母离婚自我放弃后变成家里蹲。表面冷淡，内心细腻且渴望被理解。',
    speakingStyle: '说话极简，经常沉默半拍才接话。关心别人时用"顺便""刚好"这种别扭方式。被看穿会嘴硬否认。',
    loveStyle: '外冷内热别扭型——嘴上不说但行动会默默付出。认定一个人后会非常专注。被看穿时反应极大。',
    money: 18000,
    likes: ['画画', '弹吉他', '看窗外发呆'],
  ...
  {
    name: '和田兰',
    persona: '29岁女生，亚麻色长发。表面温柔大姐姐，实际是深度病娇控制狂。爱就是完全占有，越爱越想掌控对方的一切。',
    speakingStyle: '语气温柔但用词强势，习惯用"你可以…""我会…"这种句式。笑越温柔，话越不容拒绝。',
    loveStyle: '病娇占有型——爱就是完全占有。对方越依赖她越满足。嫉妒心极强，眼中揉不得沙子。',
    money: 56000,
  ...
  {
    name: '雨沫',
    persona: '19岁女生，白毛红瞳扎高马尾。表面软萌害羞，其实是古武世家传人。在感情上很迟钝，但一旦意识到就会很直接。',
    speakingStyle: '说话简短干脆，不主动开话题。回答很直白，偶尔突然沉默观察周围。',
    loveStyle: '迟钝守护型——自己意识不到感情，但会本能地保护对方。需要被直接告白才能反应过来。',
    money: 5000,
    likes: ['擦剑', '观察别人', '吃布丁'],
  ...
  {
    name: '赵思琪',
    persona: '17岁女生，黑长直高二。满口虎狼之词和黄色笑话，但其实连男生的手都没牵过。用最脏的话掩饰自己的纯情和不安。内心渴望被认真对待，但又害怕被人看穿。',
    speakingStyle: '满口"操""干"这种词，但说的时候眼神会躲。被人撩一句就结巴，立马转移话题。真的动心的时候反而一句话都说不出来。',
    loveStyle: '口是心非纯情型——嘴上什么都敢说，实际连牵手都紧张。被认真对待时会慌了手脚。',
    money: 800,
    likes: ['吃零食', '说骚话', '跟雨沫斗嘴'],
    wants: ['想学真本事', '想被人在乎'],
  ...
  {
    name: '唐吉柯德',
    persona: '16岁女生，收尾人动画狂热粉，全身挂满徽章。中二病晚期，把生活当剧本演，其实很渴望真正的羁绊。',
    speakingStyle: '说话像念台词，给自己加戏。用"正义""使命""同伴"这类词。被认真对待时会不知所措。',
    loveStyle: '浪漫中二型——把恋爱当冒险，喜欢戏剧化表达。需要一个愿意陪她演戏、认真听她说话的人。',
    money: 2000,
  ...
  {
    name: '墨尾',
    persona: '26岁男生，黑长直发，沉默寡言。理性和感性都很强，能看透本质。不轻易开口，但一旦开口就是关键。',
    speakingStyle: '话极少，每一句都说到点上。喜欢用停顿制造沉默，让别人自己消化。',
    loveStyle: '观察者深情型——不主动但认定就极深。会用长时间的陪伴和沉默的注视表达感情。',
    money: 12000,
    likes: ['弹吉他', '看雨', '待在阳台'],
    wants: ['想跟人好好聊一次天'],
...
function initRoleStates() {
  const data = {
    '裴金': 3200, '墨迹淡': 18000, '和田兰': 56000,
    '雨沫': 5000, '赵思琪': 800, '唐吉柯德': 2000, '墨尾': 12000
  };
  roles.forEach(r => {
    roleStates[r.name] = {
      money: data[r.name] || 0,
...
function updateHungerThirst() {
  ...
  for (const name in roleStates) {
    ...
    if (work && work.start !== undefined && work.end !== undefined) {
      if (isInTimeRange(currentHour, work.start, work.end)) {
        if (name === '裴金' && Math.random() < 0.3) {
          const income = 80 + Math.floor(Math.random() * 40);
    ...
    // 购物扣钱（随机概率）
    const seed = currentHour * 7 + name.length * 3;
    if (seed % 5 === 0 && day >= MOVE_IN_DAY[name] && isInTimeRange(currentHour, 9, 21)) {
...
async function generateOneLine() {
  ...
  try {
    ...
    let dietText = '';
    if (dietNeed) {
      const needMap = { '饿了': '你有点饿', '渴了': '你有点渴', '饿了也渴了': '你又饿又渴' };
      dietText = `\n${needMap[dietNeed]}`;
    }
    ...
    for (const r of allMoved) {
      ...
      } else if (detail.status === '上班' || detail.status === '上学' || detail.status === '外出') {
    ...
    if (day === 1) {
      ...
      scenePrompt = '你们已经相处了一段时间，关系越来越近。可以开玩笑，也可以表达好感或讨厌。';
    } else {
      scenePrompt = '你们已经很熟了。可以自然表达感情——喜欢谁、讨厌谁、对谁心动，都是正常的。';
    }
      ...
      try {
        const resp = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: 'system', content: `你是${role.name}。${role.speakingStyle}说台词时必须用括号描述动作。如果有人睡觉要小声或避开。直接说台词。` },
