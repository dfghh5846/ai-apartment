...
// ================================================================
// 🔑 从环境变量读取 NVIDIA API Key（Railway 里已配置）
// ================================================================
const NVIDIA_API_KEY = process.env.NVIDIA_API_KEY;
if (!NVIDIA_API_KEY) {
  console.warn('⚠️ 未设置 NVIDIA_API_KEY 环境变量，将使用备用句模式');
}
...
const roles = [
  ...
  {
    name: '墨迹淡',
    persona: '你是25岁男性，蓝发戴眼镜，曾经是美术天才，因父母离异自我放弃后变成家里蹲。表面冷淡话少，内心极其细腻敏感。直觉敏锐，偶尔会一针见血。'
  },
  {
    name: '和田兰',
    persona: '你是29岁女性，亚麻色长发。表面温柔体贴爱照顾人，但内心极其扭曲——你用"温柔"作为控制别人的工具。你享受的不是被需要，而是"对方离不开你"的感觉。你对裴金有极强的占有欲。'
  },
  {
    name: '雨沫',
  ...
  {
    name: '赵思琪',
    persona: '你是17岁女性，黑长直琥珀眼，高二学生。极度好胜嘴硬，满口黄色笑话和脏话，自称"老子什么都懂"。但你其实完全不懂，被人撩一句就脸红结巴。'
  },
  {
    name: '唐吉柯德',
    persona: '你是16岁女性，收尾人动画的狂热粉丝。全身挂满收尾人徽章、钥匙扣、同人周边。说话像在念台词——"收尾人是正义的化身！"你觉得其他五个人是你的"同伴"，你是在"带领"他们。但你其实很善良，只是中二病晚期。'
  },
  {
    name: '墨羽',
    persona: '你是26岁男性，黑色长发及腰，沉默寡言反应慢半拍。理性和感性都极强，两者冲突导致你对外界总是慢三拍。偶尔会突然说出极其通透的话。'
  }
];
...
总字数控制在60字以内。`;
    ...
    // ================================================================
    // 🚀 调用 NVIDIA API（免费模型 meta/llama-3.1-8b-instruct）
    // ================================================================
    let reply = null;
    if (NVIDIA_API_KEY) {
      try {
        ...
        if (data.choices && data.choices.length > 0) {
          ...
          console.warn('NVIDIA API 错误:', data.error.message);
        }
      } catch (e) {
        console.warn('NVIDIA API 调用失败:', e.message);
      }
    }
    ...
    if (!reply || reply.length < 2) {
      const fallbacks = [
        '嗯…今天天气不错。', '我去倒杯水。', '你们饿不饿？', '好像要下雨了。',
        '周末有什么安排吗？', '我昨天睡得不太好。', '冰箱里没鸡蛋了。',
        '外面风有点大。', '今天心情还行。', '你们在聊什么？',
        '我刚刚在想事情。', '这个月电费好像涨了。', '有快递要拿吗？',
        '我想吃火锅。', '你们听到什么声音了吗？', '今天过得怎么样？',
        '我想出去走走。', '你们慢慢聊。', '我去阳台透透气。', '记得关窗。'
      ];
    ...
    if (pending.length > 0) {
      for (const item of pending) {
        if (reply.includes(item.slice(0, 4))) {
...
async function fetchScript() {
  try {
    ...
    document.getElementById('story').innerHTML = '<div class="empty-state" style="color:#ff6666;">❌ 无法连接服务器<br><span style="font-size:14px;color:#888;">请刷新或检查网络</span></div>';
    console.error(e);
  }
}
...
function updateProgress() {
  ...
  const count = scriptLines.length;
  prog.innerHTML = '📖 已加载 ' + count + ' 句 · 间隔 8s · ▶️ 直播中';
}

function renderStory() {
  const story = document.getElementById('story');
  if (scriptLines.length === 0) {
    if (!loadError) story.innerHTML = '<div class="empty-state">📭 还没有对话</div>';
    return;
  }
  story.innerHTML = '';
