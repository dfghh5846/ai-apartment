      ...
      try {
        const resp = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: 'system', content: `你是${role.name}，正在和室友聊天。说话要简短自然（15-25字），像真实人类一样，不要描写动作，直接说台词。` },
...
.status-panel .bar{display:inline-block;width:50px;height:6px;background:#333;border-radius:3px;margin-left:4px;vertical-align:middle}
.status-panel .bar .fill{height:100%;border-radius:3px}
.bar-hunger .fill{background:#ff8844}
.bar-thirst .fill{background:#44aaff}
</style>
</head>
<body>
<div class="wrap">
...
async function updateStatus() {
  try {
    ...
    data.status.forEach(s => {
      ...
      const hungerWidth = Math.min(100, hunger) + '%';
      const thirstWidth = Math.min(100, thirst) + '%';
      html += '<div><span class="sname">' + s.name + '</span> ' + icon + ' <span class="stat">' + s.statusText + '</span> ';
      html += '🍽️ <span class="bar bar-hunger"><span class="fill" style="width:' + hungerWidth + ';"></span></span> ';
      html += '💧 <span class="bar bar-thirst"><span class="fill" style="width:' + thirstWidth + ';"></span></span> ';
