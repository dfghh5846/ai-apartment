...
const roles = [
  ...
  },
  {
    ...
    speakingStyle: '讲话卡顿、撤回、又继续说，但一针见血',
...
let SERVER_START = new Date('2025-01-01T08:00:00Z').getTime(); // 起点：早上8点
let timelineLog = [];
...
function initRelationship() {
  roles.forEach(r => {
    relationship[r.name] = {};
    roles.forEach(o => { if (r.name !== o.name) relationship[r.name][o.name] = 0; });
  });
}
function initAll() {
  initRoleStates();
  initRelationship();
  desireStates = {}; roles.forEach(r => desireStates[r.name] = { activeDesire: null, progress: 0 });
  emotionalMemory = {}; roles.forEach(r => emotionalMemory[r.name] = []);
  diaries = {}; roles.forEach(r => diaries[r.name] = []);
  actionStates = {}; roles.forEach(r => actionStates[r.name] = { action: null, phase: 0 });
}
initAll();

// ==================== 时间系统 ====================
const MIN_PER_DAY = 24; // 现实1分钟 = 虚拟1小时
...
function checkTodayEvents(day, dateStr) {
  const events = [];
  if (HOLIDAYS[dateStr]) events.push({ type: 'holiday', name: HOLIDAYS[dateStr] });
  for (const [name, bday] of Object.entries(BIRTHDAYS)) {
    if (bday === dateStr) events.push({ type: 'birthday', name: name });
  }
  return events;
}
...
function updateWeather() {
  ...
  let weathers = [];
  if (season === '夏') weathers = ['晴天', '多云', '阴天', '小雨', '雷阵雨', '大风'];
  else if (season === '冬') weathers = ['晴天', '多云', '阴天', '小雪', '大雪', '大风'];
  else weathers = ['晴天', '多云', '阴天', '小雨', '大风'];
...
function updateHungerThirst() {
  roles.forEach(r => {
    ...
    if (!s) return;
    s.hunger = clamp(s.hunger - 0.3, 0, 100);
    s.thirst = clamp(s.thirst - 0.4, 0, 100);
    s.fatigue = clamp(s.fatigue + 0.15, 0, 100);
  });
}
...
function decideAction(role, day, hour) {
  ...
  if ((hour >= sleepStart || hour < 6) && state.fatigue > sleepTolerance) {
  ...
  if (goals.length > 0 && Math.random() < 0.2) {
    ...
    for (const goal of goals) {
      if (goalActionMap[goal]) {
        ...
        if (ACTION_DURATION[act]) return { action: act, duration: ACTION_DURATION[act] };
      }
    }
  }

  // 饥饿/口渴/疲劳 — 使用每个角色的耐受值作为阈值
  const hungerThreshold = role.hungerTolerance || 30;
  const thirstThreshold = role.thirstTolerance || 30;
  const fatigueThreshold = 70; // 疲劳超过70需要休息
  ...
  if (state.fatigue > fatigueThreshold) return { action: 'RESTING', duration: ACTION_DURATION.RESTING };
...
function sayAction(roleName, action, duration) {
  ...
  const actionDialogue = {
    SLEEPING: [
      `（打了个哈欠）好困啊...我先睡了，谁也别吵我。`,
      `（揉着眼睛）不行了...眼皮直打架，我撑不住了...`,
      `（钻进被窝）晚安...世界...我睡着了...`
    ],
    EATING: [
      `（摸着肚子）饿死我了...我去找点吃的。`,
    ...
    SOCIAL: [
      `（凑过去）嘿，聊会儿天呗？一个人待着好无聊。`,
      `（拍拍旁边的座位）过来坐嘛～聊聊呗。`,
      `（好奇地探头）你们在聊什么？加我一个！`
    ],
    CLEANING: [
      `（撸起袖子）这地方怎么这么脏？我来打扫一下。`,
      `（拿起抹布）干干净净才舒服嘛～我收拾收拾。`,
      `（哼着歌）闲着也是闲着，我擦擦桌子吧。`
    ],
    HOBBY: [
  ...
  const dialogues = actionDialogue[action] || [`（开始${action.toLowerCase()}）`];
    ...
    if (state.action && state.actionTimer > 0) {
      ...
      if (state.actionTimer === 0) {
        ...
        if (ended === 'SLEEPING') {
          ...
          state.hunger = clamp(state.hunger + 45, 0, 100);
...
function getDetailedStatus(name, hour) {
  ...
  if (state.action) {
    ...
    const map = {
      SLEEPING: '睡觉', EATING: '吃饭', DRINKING: '喝水', TOILET: '上厕所',
      RESTING: '休息', SOCIAL: '社交', CLEANING: '打扫', HOBBY: '爱好',
      WALKING: '散步', READING: '阅读', WRITING: '写作'
    };
...
function getIntroLine(name) {
  const map = {
    '裴金': '（低着头）那个…大家好，我叫裴金…请多关照…',
    '墨迹淡': '（淡淡地）……墨迹淡。',
    '何田兰': '（笑着挥手）大家好呀～我是何田兰，以后我来做饭哦～',
    '雨沫': '（抱拳）各位好，我是雨沫。',
    '赵思琪': '（抛媚眼）嗨～想姐姐了吗？嘿嘿～',
    '唐吉柯德': '（张开双臂）追尾人的勇士们！我来了！',
    '墨尾': '（愣了一下）呃…我是墨尾…嗯…就这样。'
  };
  return map[name] || '大家好。';
}
...
async function generateOneLine() {
  ...
  try {
    ...
    if (day === 1 && !introductionDone) {
      history = [
        '裴金：（低着头，声音越来越小）那个…大家好，我叫裴金…那个…以后请多关照…',
        '墨迹淡：（淡淡地扫了一眼）……墨迹淡。',
    ...
    for (const ev of todayEvents) {
      if (ev.type === 'birthday') {
        if (ev.name === role.name) eventText += '🎂 今天是你生日！大家可能会为你庆祝。';
        else eventText += `🎂 今天是 ${ev.name} 的生日，你可以说些祝福的话。`;
      } else if (ev.type === 'holiday') {
...
请说一句话（可以带括号动作描述，比如（拿起茶杯）、（低下头）），要符合你的性格，像个真人一样说话，生动自然。`;

    let reply = await callAI(role, prompt);
    if (!reply || reply.length < 2) {
      const fallbacks = {
        '裴金': ['（低头玩手指）那个…今天天气不错…', '（站起来）我…我去倒杯水…', '（小声）你们饿不饿？', '（往后退）嗯…我先回房间了…'],
        '墨迹淡': ['（头也不抬）……嗯。', '（推了推眼镜）哦。', '（转身）随便。', '（想了想）……行吧。'],
        '何田兰': ['（递过茶杯）亲爱的，要不要喝茶？', '（微笑）今天心情不错呢～', '（系上围裙）我来做饭吧。', '（歪头）你们想吃什么？'],
        '雨沫': ['（点头）没问题，交给我。', '（活动手腕）我先去练功了。', '（环视一圈）大家注意安全。', '（抱拳）有需要叫我。'],
        '赵思琪': ['（眨眨眼）想姐姐了？嘿嘿～', '（捂嘴笑）哎呀，你脸红了！', '（拍拍身边）来，坐我旁边～', '（凑近）别害羞嘛～'],
        '唐吉柯德': ['（高举双手）以追尾人之名！', '（目光坚定）这就是命运的选择！', '（望向远方）我感受到了召唤！', '（张开双臂）世界在等着我们！'],
        '墨尾': ['（挠头）呃…我…我觉得…算了。', '（低头思考）那个…其实…嗯…', '（缓缓点头）我觉得…这样挺好的。', '（抬眼）我…我先想想…']
      };
      reply = pick(fallbacks[role.name] || ['（微笑）嗯…今天天气不错。']);
    }

    // 确保有动作描述
    if (!reply.includes('（') && !reply.includes('(')) {
      const simpleActions = ['（笑了笑）', '（点头）', '（抬头）', '（轻声）', '（顿了顿）'];
      reply = pick(simpleActions) + reply;
    }
...
app.post('/api/clear', (req, res) => {
  ...
  SERVER_START = new Date('2025-01-01T08:00:00Z').getTime();
  timelineLog = [];
  currentWeather = '晴天';
  dailyTopic = '';
  storyMemory.events = [];
