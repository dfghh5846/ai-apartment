...
const roles = [
  {
    ...
    sleepStart: 21, sleepTolerance: 30, sleepDuration: 18  // 晚上9点困，低疲劳耐受力，睡9小时（18轮）
  },
  {
    ...
    background: '用数字策略合法套现32000元',
    goals: ['证明自己不是怪人', '找到一个能理解自己的人', '做出点真正厉害的事'],
    sleepStart: 2, sleepTolerance: 60, sleepDuration: 16  // 凌晨2点睡，高耐受力，睡8小时
  },
  {
...
let SERVER_START = new Date('2025-01-01T00:00:00Z').getTime(); // 固定起点
...
function startAction(roleName, action, duration) {
  const state = roleStates[roleName]; if (!state) return;
  state.action = action;
  ...
  const actionMessages = {
    SLEEPING: [
      `${roleName}打了个哈欠，眼皮直打架，嘟囔着“不行了，我困得能睡到世界末日...”`,
      `${roleName}揉着眼睛，一边往卧室走一边说“谁也别吵我，我要梦游去月球了...”`,
      `${roleName}拖沓着脚步，一头扎进被窝，含糊不清地说“晚安...世界...”`
    ],
    EATING: [
      `${roleName}闻到了饭香，肚子咕咕叫，快步走向厨房，“饿死我了，今天吃什么？”`,
      `${roleName}摸着肚子，眼巴巴地望着厨房方向，“再不吃饭我就要啃桌子了...”`,
      `${roleName}像饿了三天似的，冲到饭桌前，“我先开动了！”`
    ],
    DRINKING: [
      `${roleName}舔了舔干裂的嘴唇，拿起水杯吨吨吨喝了几大口，“啊...活过来了...”`,
      `${roleName}口渴得不行，赶紧去找水，“再不喝水我就要变成沙漠了。”`
    ],
    TOILET: [
      `${roleName}脸色一变，捂着肚子快步走向卫生间，“不好意思，我憋不住了...”`,
      `${roleName}一声不吭，直奔洗手间，留下“砰”的关门声。`
    ],
    RESTING: [
      `${roleName}瘫在沙发上一动不动，长舒一口气，“让我躺会儿，谁也别叫我...”`,
      `${roleName}闭上眼睛，靠在椅背上，喃喃道“世界太吵了，我需要静静...”`
    ],
    SOCIAL: [
      `${roleName}凑到别人身边，眨眨眼，“嘿，聊会儿天呗？”`,
      `${roleName}笑着拍了拍旁边的座位，“过来坐嘛，一个人待着多没意思。”`
    ],
    CLEANING: [
      `${roleName}撸起袖子，拿着抹布开始擦擦擦，“这地方怎么这么脏？我受不了了！”`,
      `${roleName}像田螺姑娘附体一样，哼着歌收拾房间，“干干净净才舒服嘛。”`
    ],
    HOBBY: [
      `${roleName}拿出一本书/画笔/乐器，脸上露出满足的笑容，“终于可以享受一下我的小爱好了...”`,
      `${roleName}沉浸在兴趣里，忘掉了所有烦恼，“这才是生活啊～”`
    ],
    WALKING: [
      `${roleName}伸了个懒腰，决定出去走走，“闷死了，去外面透透气。”`,
      `${roleName}一边散步一边哼着小调，心情看起来不错。`
    ],
    READING: [
      `${roleName}翻开书页，眼神专注，“这本书真的有意思，越看越停不下来...”`,
      `${roleName}捧着书本，完全沉浸在故事里，雷打不动。`
    ],
    WRITING: [
      `${roleName}拿起笔，在纸上涂涂写写，眉头时而紧锁时而舒展，“嗯...这句怎么改好呢...”`,
      `${roleName}对着笔记本发呆，突然灵光一闪，飞快地写下一大段。`
    ]
  };
  const messages = actionMessages[action] || [`${roleName}开始${action.toLowerCase()}`];
...
app.post('/api/clear', (req, res) => { history = []; currentIdx = 0; introductionDone = false; SERVER_START = new Date('2025-01-01T00:00:00Z').getTime(); timelineLog = []; dailyActivities = {}; initRoleStates(); initRelationship(); initMoods(); initDesireStates(); initEmotionalMemory(); initDiaries(); initActionStates(); res.json({ status: 'cleared' }); });
