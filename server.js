...
async function generateOneLine() {
  ...
  try {
    ...
    // 检测动作关键词，但仅用于更新欲望，不触发新动作（避免重复）
    const actionKeywords = { '睡': 'SLEEPING', '睡觉': 'SLEEPING', '休息': 'RESTING', '吃饭': 'EATING', '吃': 'EATING', '喝水': 'DRINKING', '看书': 'READING', '读书': 'READING', '散步': 'WALKING', '写': 'WRITING', '聊天': 'SOCIAL', '打扫': 'CLEANING' };
    for (const [kw, act] of Object.entries(actionKeywords)) {
      if (reply.includes(kw)) {
        const s = roleStates[role.name];
        if (s && !s.action && !s.isSleeping) {
          s.desire = act;
