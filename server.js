...
async function generateOneLine() {
  ...
  try {
    ...
    // 🔧 修复：根据亲密度调整说话方式，第1天强调初次见面
    let intimacyNote = '';
    if (day === 1) {
      intimacyNote = '今天是你们第一次见面，你们还不认识对方，说话要客气、礼貌，保持距离。';
    } else if (day <= 3) {
      intimacyNote = '你们刚认识不久，还不太熟，说话要礼貌一些。';
    } else if (day <= 7) {
      intimacyNote = '你们开始熟悉了，可以偶尔开开玩笑，但不要太过。';
    } else {
      intimacyNote = '你们已经相处一段时间了，可以随意聊天。';
    }
...
请以${role.name}的身份说一句简短的日常对话（15-25字），像真人聊天一样自然，不要长篇大论，不要编造过去的共同经历。`;
