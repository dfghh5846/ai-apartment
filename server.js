...
.progress-info{display:flex;gap:12px;font-size:12px;color:#888;padding:4px 0;}
</style>
</head>
<body>
<div class="wrap">
...
function updateProgress() {
  ...
  if (totalLines === 0) {
    prog.textContent = '📭 暂无剧本';
    return;
  }
  const progress = Math.min(currentIndex + 1, totalLines);
  const percent = Math.round((progress / totalLines) * 100);
  const status = isPlaying ? '▶️ 直播中' : '⏸️ 已暂停';
  prog.textContent = \`📖 \${progress}/\${totalLines} 句 (\${percent}%) · 间隔 \${interval}s · \${status}\`;
}
...
function openAdmin() {
  ...
  content.innerHTML = \`
    ...
    <button onclick="adminLogin()">登录</button>
    <div id="adminPanel" style="display:none;margin-top:12px;">
      <label>播放间隔（秒）：<input type="number" id="editInterval" value="\${interval}" /></label>
      <button onclick="updateInterval()">⏱️ 更新间隔</button>
      <hr style="margin:12px 0;border-color:#444;" />
