...
const MOVE_IN_DAY = { '裴金': 1, '墨迹淡': 1, '和田兰': 1, '雨沫': 2, '赵思琪': 3, '唐吉柯德': 5, '墨羽': 15 };
const WORK_SCHEDULE = { '裴金': null, '墨迹淡': null, '和田兰': { start: 10, end: 19 }, '雨沫': { start: 8, end: 17 }, '赵思琪': { start: 8, end: 17 }, '唐吉柯德': { start: 8, end: 17 }, '墨羽': null };
const SLEEP_SCHEDULE = { '裴金': { start: 22, end: 6 }, '墨迹淡': { start: 2, end: 10 }, '和田兰': { start: 23, end: 7 }, '雨沫': { start: 21, end: 6 }, '赵思琪': { start: 23, end: 6 }, '唐吉柯德': { start: 23, end: 7 }, '墨羽': { start: 1, end: 9 } };
...
// ================================================================
const roles = [
  { name: '裴金', persona: '24岁女性，金发短发圆框眼镜，线上心理咨询师（在家办公）。敏感易内疚，不敢拒绝别人。' },
  { name: '墨迹淡', persona: '25岁男性，蓝发戴眼镜，曾是美术天才，因父母离异自我放弃变成家里蹲。表面冷淡，内心细腻。' },
  { name: '和田兰', persona: '29岁女性，亚麻色长发。表面温柔体贴，内心占有欲极强，用"温柔"作为控制工具。对裴金有极强占有欲。' },
  { name: '雨沫', persona: '19岁女性，白毛红瞳扎高马尾，古武传人，警惕性高，不暴露身份。' },
  { name: '赵思琪', persona: '17岁女性，黑长直琥珀眼，高二学生。极度好胜嘴硬，满口黄色笑话，自称"老子什么都懂"，但其实完全不懂。' },
  { name: '唐吉柯德', persona: '16岁女性，收尾人动画狂热粉丝，全身挂满徽章和周边。说话像念台词，觉得自己是正义化身，正在"带领"同伴。' },
  { name: '墨羽', persona: '26岁男性，黑色长发及腰，沉默寡言反应慢半拍，偶尔会突然说出极其通透的话。' }
];
...
function getIntroLine(roleName) {
  const intros = {
    ...
    '唐吉柯德': '（推门而入，披风一甩）正义的收尾人，唐吉柯德，参上！你们就是我的新同伴吧？',
    '墨羽': '……(沉默几秒后)墨羽。……住这里。'
  };
    ...
    if (NVIDIA_API_KEY) {
      try {
        ...
        if (data.choices && data.choices.length > 0) {
          reply = data.choices[0].message.content.trim().replace(/^["']|["']$/g, '');
        }
...
<div class="modal">
<div class="modal-header"><h2>📋 菜单</h2><button class="modal-close" onclick="closeMenu()">✕</button></div>
<div style="display:flex;flex-direction:column;gap:8px;"><button onclick="openAdmin()">🔐 创作者后台</button></div>
</div>
</div>
<div class="modal-overlay" id="adminModal">
<div class="modal">
<div class="modal-header"><h2>🔐 创作者后台</h2><button class="modal-close" onclick="closeAdmin()">✕</button></div>
...
async function fetchScript() {
  try {
    ...
    const prog = document.getElementById('progressDisplay');
    prog.innerHTML = '📖 已加载 ' + scriptLines.length + ' 句 · 间隔 8s · ▶️ 直播中';
...
function toggleMenu() { document.getElementById('menuModal').classList.toggle('open'); }
function closeMenu() { document.getElementById('menuModal').classList.remove('open'); }
function openAdmin() {
  closeMenu();
  document.getElementById('adminModal').classList.add('open');
  document.getElementById('adminContent').innerHTML = '<input type="password" id="adminPwd" placeholder="密码" /><button onclick="adminLogin()">登录</button><div id="adminPanel" style="display:none;margin-top:12px;"><button onclick="clearHistory()" style="background:#6a3a3a;">🗑️ 清空记录</button><div id="saveMsg"></div></div>';
}
...
async function adminLogin() {
  ...
  if (pwd === 'admin123') {
    ...
    document.getElementById('saveMsg').textContent = '✅ 已登录';
  } else { alert('密码错误'); }
}
async function clearHistory() {
  ...
  try {
    ...
    if (res.ok) { scriptLines = []; fetchScript(); document.getElementById('saveMsg').textContent = '✅ 已清空'; }
  } catch(e) { alert('失败'); }
}
fetchScript();
