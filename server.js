...
if
      const content = line.slice(colonIdx + 1);
常量clsMap={'裴金'：'Pei'，'墨迹淡'：'Moji'，'和田兰'：'和田‘、‘雨沫'：'Yumo'，'赵思琪'：'四七‘、‘墨羽'：'Moyu'常量cls=clsMap[名称]||"；；
常量cls=clsMap[名称]||"；
Const div=document.createElement('div')；div=document.createElement('div'div.className='line'+cls；；
div.className='line'div.innerHTML='<div class="action">'+名称+'</div>'+内容；cls；
div.innerHTML='<div class="action">'+名称+'</div>'if(！ 确认('确定要清空所有聊天记录吗？ 此操作不可撤销。 '))返回；内容；
...
if(！确认('确定要清空所有聊天记录吗？此操作不可撤销。'常量res=等待提取('/api/clear'，{method：'POST'})；)返回；
  try {
常量res=等待提取('/api/clear'，{method：'POST'if(RES.OK){story.innerHTML='<div class="empty-state">🗑️ 已清空，重新生成中...</div>'；；
    if (res.ok) {
      lastLength = 0;
story.innerHTML='<div class="empty-state">🗑️ 已清空，重新生成中...</div>'count.textContent="0"；
警惕的('清空失败，请重试')；.textContent=“0”；
      setTimeout(fetchHistory, 2000);
    } else {
警惕的('清空失败，请重试')警惕的('网络错误，请检查连接')；
    }
  } catch(e) {
警惕的('网络错误，请检查连接')常数res=等待提取('/api/status')；
  }
    }

async function showStatus() {
  try {
常数res=等待提取('/api/status'Const data=await res.json()；常量virtualDate=data.virtualDate||"；
    const data = await res.json();
    const status = data.status || [];
    const events = data.events || [];
    const currentDay = data.currentDay || 0;
    const virtualDate = data.virtualDate || '';
让html='<div class="time-display">🏠 公寓第'+currentday+'天('+virtualDate+')</div>'； html='<div class="time-display">🏠 公寓第'+currentday+'天('+virtualDate+')</div>'status.forEach(s=>{
常数色图={'裴金'：'#ffddaa'，'墨迹淡'：'#99ccff'，'和田兰'：'#ffb8cc'，'雨沫'：'#ff88aa'，'赵思琪'：'#ffaa66'，'墨羽'：'#AABBDD'}；.forEach(s=>{
常数色图={'裴金'：'#ffddaa'，'墨迹淡'：'#99ccff'，'和田兰'：'#ffb8cc'，'雨沫'：'#ff88aa'，'赵思琪'：'#ffaa66'，'墨羽'：'#AABBDD'常量颜色=颜色映射[s.name]||'#666'；；
常量颜色=颜色映射[s.name]||'#666'常量homeIcon=s.movedin？(s.家？'🏠'：'😴')：'🚪'；
常量homeIcon=s.movedin？(s.家？'🏠'：'😴')：'🚪'HTML+='<div class="角色状态"style="border-left-color：'+color+'"><div class="rname"style="color：'+color+'">'+s.名称+'</div><div class="rlast">💬 's+。最后一行+'</div><div class="revent">📌 参与事件：'+(s.artventedInEvent？'✅ 有'：'❌ 无')+'</div><div class="rtime">'+homeIcon+"+(s.移动？(s.ishome？'在家'：'从……里面出去out/已睡')：'尚未搬入')+'·发言'+s.TotalLines+'次</div></div>'；
HTML+='<div class="角色状态"style="border-left-color：'+color+'"><div class="rname"style="color：'+color+'">'+s.name+'</div><div class="rlast">💬 '+s.lastLine+'</div><div class="revent">📌 参与事件：'+(s.artventedInEvent？'✅ 有'：'❌ 无')+'</div><div class="rtime">'+homeIcon+"+(s.movedin？(s.ishome？'在家'：'从……里面出去out/已睡')：'尚未搬入')+'·发言'+s.TotalLines+'次</div></div>'；
HTML+='<div style="margin-top:12px；填充：8px；背景：#1a1a27；边框半径：8px；字体大小：12px；颜色：#AAA"><div style="color：#ffd399；font-weight:600；margin-bottom:4px">📜 近期事件</div>'+事件。map(e=>'<div>·'+e+'</div>').join(")+'</div>'；})；
    if (events.length > 0) {
HTML+='<div style="margin-top:12px；填充：8px；背景：#1a1a27；边框半径：8px；字体大小：12px；颜色：#aaa"><div style="color：#ffd399；font-weight:600；margin-bottom:4px">📜 近期事件</div>'+events.map(e=>'<div>·'+e+'</div>').join(")+'</div>'；
    文件。getElementById('statusContent')。innerHTML=html；
文件。getElementById('statusContent'文件.getElementById('modal').classList.Add('open')；.innerHTML=html；
文件。getElementById('modal').classList.add('open'}渔获物(e){警惕的('获取状态失败')；
  } catch(e) {
警惕的('获取状态失败')文件。getElementById('modal').classList.remove('open')；
  }
    }

function closeModal() {
文件。getElementById('modal').classList.remove('open'fetchHistory()；；
</script>
