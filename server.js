...
function getAvailableRoles(day, hour) {
  ...
  try {
    ...
    const actionState = actionStates[role.name];
    let actionText = '';
    if (actionState && actionState.action) {
      const phases = ACTION_STATES[actionState.action]?.phases || [];
...
⚠️ 重要规则：
1. 只输出你说的话，不要用括号、不要旁白、不要内心独白。
2. 如果你想表达动作，直接用话语说出来，例如"我去吃饭了""我去睡觉了""我去上个厕所"。
3. 像真人一样自然地聊天。
4. 禁止英文、禁止重复。
5. 字数15-35字。`;
      ...
      try {
        const resp = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [{ 
              role: 'system', 
              content: `你是${role.name}，${genderText}。${role.speakingStyle}。直接说你说的话，可以自然地表达你的动作（比如“我去吃饭了”“我去睡觉了”），但绝对不要用括号或旁白描述。像真人一样说话，禁止英文。` 
