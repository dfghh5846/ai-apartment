...
const roles = [
  ...
  },
  {
    ...
    speakingStyle: '甜腻温柔，话里有刺，喜欢用"亲爱的""宝贝"来称呼别人，语气越温柔内容越可怕',
  ...
  },
  {
    ...
    speakingStyle: '口无遮拦，喜欢开黄腔撩人，但一被反撩就脸红结巴，喜欢说“想姐姐了？”“嘿嘿～”“你、你胡说什么！”',
  ...
  },
  {
    ...
    speakingStyle: '充满激情，把游戏台词带入日常，说话夸张，喜欢说“以追尾人之名！”“这就是命运的选择！”',
  ...
  },
  {
    ...
    speakingStyle: '讲话卡顿、撤回、又继续说，但一针见血，让每个人都不反感，喜欢说“呃…”“那个…”“算了…”“我觉得…”',
...
async function callAI(role, prompt) { if (!NVIDIA_API_KEY) return null; try { const resp = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${NVIDIA_API_KEY}` }, body: JSON.stringify({ model: 'deepseek-ai/deepseek-v4-flash', messages: [{ role: 'system', content: `你叫${role.name}，${role.gender}，你的性格是：${role.persona}。你的说话风格是：${role.speakingStyle}。不要用括号描述动作，直接用角色身份说话，要像真人一样自然、有情绪。不要说“你好”“再见”这种生硬的词，要像日常聊天。禁止英文。` }, { role: 'user', content: prompt }], temperature: 1.0, max_tokens: 120 }) }); if (!resp.ok) throw new Error(`HTTP ${resp.status}`); const data = await resp.json(); if (data.choices && data.choices.length > 0) { let reply = data.choices[0].message.content.trim(); reply = reply.replace(/<\|eot_id\|>/g, '').replace(/<\|start_header_id\|>/g, '').replace(/<\|end_header_id\|>/g, ''); reply = reply.replace(/[a-zA-Z]/g, '').trim(); reply = reply.replace(/[（(][^）)]*[）)]/g, '').trim(); return reply; } return null; } catch (e) { console.warn('API错误:', e.message); return null; } }
...
请用非常自然、口语化的中文说一句话（15-35字），要符合你的性格，像个真人一样说话。不要用括号或旁白，直接说出你想说的话。`;
    ...
    let reply = await callAI(role, prompt); if (!reply || reply.length < 2) {
      const fallbacks = {
        '裴金': ['那个…今天天气不错…', '我…我去倒杯水…', '你们饿不饿？'],
        '墨迹淡': ['……嗯。', '哦。', '随便。'],
        '何田兰': ['亲爱的，要不要喝茶？', '今天心情不错呢～', '我来做饭吧。'],
        '雨沫': ['没问题，交给我。', '我先去练功了。', '大家注意安全。'],
        '赵思琪': ['想姐姐了？嘿嘿～', '哎呀，你脸红了！', '来，坐我旁边～'],
        '唐吉柯德': ['以追尾人之名！', '这就是命运的选择！', '我感受到了召唤！'],
        '墨尾': ['呃…我…我觉得…算了。', '那个…其实…嗯…', '我觉得…这样挺好的。']
      };
